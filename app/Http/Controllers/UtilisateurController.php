...
class UtilisateurController extends Controller
{
        ...
        try {
            ...
            if (!$sexe) {
                return response()->json([
                    'error' => 'Le sexe avec l\'ID spécifié n\'existe pas.'
                ], 404);
            }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($this->tokenService->isTokenValid($token)) {
            ...
            return response()->json(['message' => 'Session expirée'], 403);
        }
    }
    ...
    {
        try {
            $this->utilisateurService->validerCompte($request->input('token'));
            return response()->json(['message' => 'Compte validé avec succès !'], 200);
        } catch (\Exception $e) {
            return response()->json(['message' => $e->getMessage()], 400);
        }
    }
    ...
    public function validationPin(Request $request)
    {
        try {
            ...
            return response()->json(['message' => 'Code PIN non valide ou expiré'], 400);
        }
    }
